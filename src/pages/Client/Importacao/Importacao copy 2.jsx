...
export default function Importacao() {
  ...
  const podeEnviar = useMemo(() => {
    ...
    if (isValeTransporte) {
      return linhasValidadas.every(r => !r.bloqueado)
    }
  ...
  const confirmarEnvio = async () => {
    ...
    try {
      ...
      // NÃO ENVIA erros e linhas_com_erro - o backend espera receber apenas dados válidos
      // Remove errors e linhas_com_erro do objeto
  ...
  return (
    <PageLayout title="Importação" subtitle="Importe arquivos .txt, .csv ou .xlsx">
      <div className="importacao-container">
                  ...
                  <div className="error-details-tooltip">
                    ...
                    {linhasComErroBackend.map((erro, idx) => (
                      <div key={idx} className="error-item">
                        ...
                        <span className="error-message">{erro.mensagem}</span>
                        {erro.detalhes?.cpf && (
                          <span className="error-cpf">CPF: {erro.detalhes.cpf}</span>
                        )}
                        {erro.detalhes?.nome && (
                          <span className="error-nome">Nome: {erro.detalhes.nome}</span>
                        )}
                      </div>
                    ))}
                  </div>
        ...
        {/* Modals... (mantenha os modals como estavam) */}
